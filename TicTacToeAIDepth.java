...
//Here we have considered depth so if at less depth then more priority should be given
public class TicTacToeAIDepth {
	...
	public static void takeInput(char grid[],char input){
	    ...
	    int i = sc.nextInt();
	    while(grid[i-1]!='-') {
	    	System.out.println("Please enter in blank space");
	    	System.out.println("Where you want to place "+input);
		    i = sc.nextInt();
	    }
	    grid[i-1]=input;
	...
	public static int noOfWays(char grid[],char input) {
		int ans = 8;
		for(int i=0;i<3;i++) {
			if(grid[i]!='-' && grid[i]!=input) {
				ans-=1;
				break;
			}
		}
