...
class Nodee{
    ...
    int prev;
    int depth;
    public Nodee(int grid[][],int sr,int sc,int prev,int depth){
        this.grid=grid;
        this.sr=sr;
        this.sc=sc;
        this.prev=prev;
...
public class EightPuzzleBFS {
	...
	public static void eightPuzzle(int sr,int sc,int initialState[][],int goalState[][]) {
        ...
        q.add(new Nodee(newState,sr,sc,-1,0));
        while(!q.isEmpty()){
            int count = q.size();
            for(int i=0;i<count;i++){
                ...
                if(prev!=1 && scol>0) {//left
        			...
        			q.add(new Nodee(newstate,srow,scol-1,3,node.depth+1));
        			//Backtracking
        		...
        		if(prev!=2 && srow>0) {//up
        			...
        			q.add(new Nodee(newstate,srow-1,scol,0,node.depth+1));
        			//Backtracking
        			temp = state[srow][scol];
        			state[srow][scol] = 0;
        		...
        		if(prev!=3 && scol<2) {//right
        			...
        			q.add(new Nodee(newstate,srow,scol+1,1,node.depth+1));
        			//Backtracking
        			temp = state[srow][scol];
        			state[srow][scol] = 0;
        			state[srow][scol+1] = temp;
        		...
        		if(prev!=0 && srow<2) {//down
        			...
        			q.add(new Nodee(newstate,srow+1,scol,2,node.depth+1));
        			//Backtracking
        			temp = state[srow][scol];
        			state[srow][scol] = 0;
        			state[srow+1][scol] = temp;
